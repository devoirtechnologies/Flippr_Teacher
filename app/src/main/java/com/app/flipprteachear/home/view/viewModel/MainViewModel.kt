...
class MainViewModel :ViewModel() {
   ...
   private val liveDataCheckBox= MutableLiveData<ResultWrapper<PojoAllUpdate>>()
    ...
    fun studentProfileApi(token: String, userID: String): MutableLiveData<ResultWrapper<PojoStudentProfile> > {
        viewModelScope.launch(Dispatchers.IO){
            try {
                ...
                if(e is IOException)  liveStudentProfile.postValue(ResultWrapper.NetworkError)  else liveStudentProfile.postValue(ResultWrapper.GenericError(
                    ErrorResponse(-1, e.message)
                ))
